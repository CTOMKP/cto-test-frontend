...
export const ListingDetail: React.FC = () => {
  ...
  const [recentTradeKeys, setRecentTradeKeys] = useState<Set<string>>(new Set());
  ...
  useEffect(() => {
    ...
    socket.on('trades.update', (payload: any) => {
      ...
      setTrades((prev) => {
        ...
        incoming.forEach(addTrade);
        prev.forEach(addTrade);
      ...
      for (const t of incoming) {
        const key = buildTradeKey(t);
        if (!knownTradeKeys.current.has(key)) {
          newKeys.push(key);
          knownTradeKeys.current.add(key);
        }
      }

      if (newKeys.length > 0) {
        setRecentTradeKeys((prev) => {
  ...
  const buildTradeKey = (trade: TradeEvent) => {
    const hash = trade.txHash || trade.transactionHash;
    if (hash) return hash;
    const ts = trade.timestamp ? new Date(trade.timestamp as any).getTime() : 0;
    const amount = Number(trade.amount ?? trade.amountToken ?? 0);
    const price = Number(trade.price ?? trade.priceUSD ?? 0);
          ...
          <div className="px-4 py-3">
            ...
            {!tradesLoading && !tradesError && dedupedTrades.length > 0 && (
              <div className="overflow-hidden">
                <div className="grid grid-cols-[90px_70px_1fr_1fr_1fr_1fr_1fr] gap-2 text-xs text-gray-500 border-b pb-2">
                  <div>Time</div>
                  <div>Type</div>
                ...
                <div className="mt-2 max-h-[420px] overflow-y-auto space-y-1">
                  <AnimatePresence initial={false}>
                    {dedupedTrades.map((trade, idx) => {
                      ...
                      const isNew = recentTradeKeys.has(tradeKey);
                      ...
                      return (
                        <motion.div
                          ...
                          className={`grid grid-cols-[90px_70px_1fr_1fr_1fr_1fr_1fr] items-center gap-2 px-2 py-2 rounded transition-colors ${
                            isNew
                              ? isBuy
                                ? 'bg-emerald-50'
                                : 'bg-rose-50'
