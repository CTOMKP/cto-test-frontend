...
export class CircleWalletService {
  ...
  constructor() {
    ...
    this.backendUrl = process.env.REACT_APP_BACKEND_URL || 'https://api.ctomarketplace.com';
  ...
  async getUserToken(userId: string): Promise<{ userToken: string; encryptionKey: string }> {
    try {
      ...
      console.log('🔍 Full endpoint:', `${this.backendUrl}/api/circle/users/token`);
  ...
  async createWallet(userId: string, userEmail: string, description?: string): Promise<CircleWallet> {
    try {
      ...
      }, { timeout: 15000 }); // Allow up to 15s for PIN/wallet creation to avoid premature aborts
