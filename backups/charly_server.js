...
app.get('/api/circle/wallets/:walletId/transactions', async (req, res) => {
  ...
  try {
    ...
    try {
      const response = await axios.get(`${CIRCLE_API_BASE}/wallets/${walletId}/transactions`, {
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${CIRCLE_API_KEY}`,
          'X-User-Token': freshUserToken
      ...
      if (response.data && response.data.data && response.data.data.transactions) {
        const transactions = response.data.data.transactions.map((tx) => ({
          ...
          description: tx.description || `${tx.type || 'Transaction'} ${tx.amount || '0'} ${tx.currency || 'USDC'}`,
      ...
      // Circle API doesn't support transactions endpoint or returned an error
      console.log('⚠️ Circle API transactions endpoint not available or returned error:', circleApiError.response?.status, circleApiError.response?.data);
