...
export const useAuth = () => {
  ...
  const checkAuthStatus = useCallback(async () => {
    ...
    try {
      ...
      // OPTIMIZATION: Immediate authentication check for users with wallets
      if (storedEmail && storedWalletId) {
        // User has email and wallet - they are definitely authenticated
        console.log('✅ Found user with wallet, immediately authenticated');
        const user: User = {
          id: storedEmail,
          email: storedEmail,
        ...
        console.log('✅ Authentication state set immediately from wallet data');
        return;
      }
