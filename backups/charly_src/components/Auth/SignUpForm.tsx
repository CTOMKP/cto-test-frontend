...
export const SignUpForm: React.FC = () => {
  ...
  // Check for Google OAuth user on component mount and from navigation state
  useEffect(() => {
    const checkUserStatus = async () => {
      ...
      // Check if we came from Google OAuth navigation
      ...
      if (isFromGoogleOAuth && userEmailFromState) {
        ...
        // If user already has a wallet, redirect immediately
        if (walletId) {
        ...
        // Clear the navigation state to prevent showing it again on refresh
        window.history.replaceState({}, document.title);
      } else if (storedEmail && authToken && authToken.startsWith('google_token_')) {
        setIsGoogleOAuthUser(true);
        setGoogleUserEmail(storedEmail);
        console.log('🔄 Detected Google OAuth user from localStorage:', storedEmail);
      ...
      if (isGoogleOAuthUser) {
        ...
        try {
          ...
          setTimeout(() => {
            ...
            console.log('🔄 ROUTES.profile value:', ROUTES.profile);
            console.log('🔄 navigate function:', typeof navigate);
            console.log('🔄 About to call navigate...');
            
            try {
              navigate(ROUTES.profile);
              console.log('✅ navigate() called successfully for Google OAuth user');
            } catch (navError) {
              console.error('❌ navigate() failed for Google OAuth user:', navError);
              // Fallback: try window.location
              console.log('🔄 Trying window.location fallback for Google OAuth user...');
          ...
          if (errorMessage.includes('PIN setup required before wallet creation')) {
            ...
            console.log('🔄 PIN setup in progress for Google OAuth user, waiting for completion...');
            toast('PIN setup in progress. Please complete the PIN setup to continue.');
            // Don't redirect - let the PIN setup complete
          } else {
            // Check if wallet was actually created despite the error (fallback check)
            try {
              console.log('🔄 Checking if wallet was actually created for Google OAuth user despite error...');
              ...
              if (walletsResponse.data.success && walletsResponse.data.wallets && walletsResponse.data.wallets.length > 0) {
                console.log('✅ Wallet was actually created for Google OAuth user! Getting wallet details...');
        ...
        if (authResponse?.user && authResponse.user.id) {
          ...
          try {
            ...
            // This ensures the user is properly authenticated for the profile page
            ...
            console.log('🔄 localStorage after wallet creation:');
            console.log('🔄 cto_wallet_id:', localStorage.getItem('cto_wallet_id'));
            console.log('🔄 cto_user_email:', localStorage.getItem('cto_user_email'));
            console.log('🔄 cto_user_created:', localStorage.getItem('cto_user_created'));
            console.log('🔄 cto_wallet_address:', localStorage.getItem('cto_wallet_address'));
            console.log('🔄 cto_auth_token:', localStorage.getItem('cto_auth_token'));
            
            setWalletAddress(wallet.address);
            toast.success('Account and wallet created successfully! Redirecting to dashboard...');
            
            // CRITICAL FIX: Update authentication state before redirecting
            console.log('🔄 Updating authentication state after wallet creation...');
            
            // OPTIMIZATION: Immediately set authentication state to prevent signup page flash
            const user = {
              id: data.email,
              email: data.email,
              walletId: wallet.id,
            ...
            console.log('🔄 Debug: localStorage contents before redirect:');
            console.log('🔄 cto_user_email:', localStorage.getItem('cto_user_email'));
            console.log('🔄 cto_auth_token:', localStorage.getItem('cto_auth_token'));
            console.log('🔄 cto_wallet_id:', localStorage.getItem('cto_wallet_id'));
            
            // OPTIMIZATION: Immediate redirect without delay to prevent signup page flash
            console.log('✅ Redirecting to dashboard immediately after successful wallet creation');
            console.log('🔄 ROUTES.profile value:', ROUTES.profile);
            console.log('🔄 navigate function:', typeof navigate);
            console.log('🔄 About to call navigate...');
            ...
            if (errorMessage.includes('PIN setup required before wallet creation')) {
              // This is normal - PIN setup is in progress, don't redirect yet
              console.log('🔄 PIN setup in progress, waiting for completion...');
              toast('PIN setup in progress. Please complete the PIN setup to continue.');
              // Don't redirect - let the PIN setup complete
            } else if (authResponse?.user?.id) {
              // Account was created but wallet creation failed for other reasons
              console.log('⚠️ Account created but wallet creation failed:', errorMessage);
              
              // Check if wallet was actually created despite the error (fallback check)
              try {
                console.log('🔄 Checking if wallet was actually created despite error...');
                const backendUrl = process.env.REACT_APP_BACKEND_URL || 'http://localhost:3001';
                const walletsResponse = await axios.get(`${backendUrl}/api/circle/users/${data.email}/wallets`);
                
                if (walletsResponse.data.success && walletsResponse.data.wallets && walletsResponse.data.wallets.length > 0) {
                  console.log('✅ Wallet was actually created! Getting wallet details...');
                  const walletData = walletsResponse.data.wallets[0];
              ...
              // Don't redirect - let user retry wallet creation
            } else {
